...
export const signin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!isMatch) {
      res.status(400).json({ message: "Invalid Crendentials, wrong password" });
      return;
    }
...
export const initiateAdmin = async (
  ...
  try {
    ...
    if (count > 0) {
      res.status(400).json({
        massage:
          "We can only have 1 admin user, if you want to create new admin user, please delete the user manually from the datebase",
      });
      return;
    }
